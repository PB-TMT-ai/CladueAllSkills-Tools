"""
generate_consolidated_map_v10.py
V10 consolidated map — yellow choropleth, cleaned-up sidebar.
- Choropleth: states colored by FY27 sales (YELLOW gradient) from Market Share tab
- Factory icons at plant locations with numbered labels
- State grade labels on map
- NO plant directory, NO state table
- Slim sidebar: grade breakdown totals only (old color key removed)
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "consolidated_supply_map_v10.png")
...
def generate():
    ...
    plt.savefig(OUTPUT_FILE, dpi=300, bbox_inches='tight', facecolor='white',
                pad_inches=0.3)
    plt.close()
    print(f"V10 map saved: {OUTPUT_FILE}")
