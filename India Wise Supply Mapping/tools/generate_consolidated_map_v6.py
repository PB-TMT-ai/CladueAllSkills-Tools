"""
generate_consolidated_map_v6.py
V6 consolidated map — same as V5 but with doubled state grade label sizes.
- Choropleth: states colored by total supply volume (green gradient)
- Factory icons at plant locations with numbered labels
- State grade labels on map: 2x larger font (abbreviation=15, grade line=11)
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "consolidated_supply_map_v6.png")
...
def generate():
    ...
    # ── State grade labels — 2x size of V5 ───────────────────────────
    # V5: abbreviation=7.5, grade_line=5.5, spacing=0.45
    # V6: abbreviation=15,  grade_line=11,  spacing=0.9
    for state, total in state_totals.items():
        ...
        # State abbreviation — doubled from 7.5 → 15
        ax_map.text(
            cx, cy + 0.9, abbrev,
        ...
        # Grade breakdown line — doubled from 5.5 → 11
        if grade_line:
            ax_map.text(
                cx, cy - 0.9, grade_line,
                fontsize=11, ha='center', va='center', fontweight='bold',
                color='#37474F',
                bbox=dict(facecolor='white', alpha=0.85, edgecolor='none',
                          pad=2.0, boxstyle='round,pad=0.2'),
                zorder=8,
            )
    ...
    if supply_states:
        ...
        cbar = fig.colorbar(sm, ax=ax_map, location='bottom', shrink=0.5,
                            pad=0.03, aspect=30)
        cbar.set_label('Total Inbound Supply (MT)', fontsize=12, fontweight='bold')
    ...
    plt.savefig(OUTPUT_FILE, dpi=300, bbox_inches='tight', facecolor='white',
                pad_inches=0.3)
    plt.close()
    print(f"V6 map saved: {OUTPUT_FILE}")
