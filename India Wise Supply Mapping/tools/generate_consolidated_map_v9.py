"""
generate_consolidated_map_v9.py
V9 consolidated map — updated data version.
- Choropleth: states colored by FY27 sales (green gradient) from Market Share tab
- Factory icons at plant locations with numbered labels
- State grade labels on map: 25%% larger than V6 (abbreviation=18.75, grade line=13.75)
- NO plant directory, NO state table
- Slim sidebar: color key + grade breakdown totals
- Plant locations updated per district-wise screenshot
"""
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "consolidated_supply_map_v9.png")
...
# ── State label positions (lon, lat) — spread further for larger labels ─
STATE_LABEL_POS = {
...
# ── Plant locations (lon, lat) — updated per district-wise screenshot ──
PLANT_LOCATIONS = {
    ...
    "Ambashakti Industries Limited": (79.8, 27.2),            # Sikandarabad, Uttar Pradesh
...
PLANT_DISPLAY_OFFSETS = {
    # Raipur, CG — API Ispat + SKA Ispat + Real Ispat overlap
    "SKA Ispat Private Limited": (1.8, 1.5),
    "Real Ispat": (1.8, -0.5),
    "API Ispat And Powertech Private Limited": (-1.8, 0.5),
}
...
def generate():
    ...
    # ── Layout: larger figure to fit doubled label sizes ──────────────
    ...
    # ── Choropleth — colored by FY27 sales ────────────────────────────
    ...
    # ── State grade labels — NO abbreviations, grade line +10% from V9 ──
    # V6 grade_line=11 → V9 was 13.75 (+25%) → now 15.125 (+10% more)
    for state, total in state_totals.items():
        ...
        # Build grade parts — only show grades that have volume
        parts = []
        if f550 > 0:
            parts.append(f"550:{_fmt(f550)}")
        if f_oh > 0:
            parts.append(f"OH:{_fmt(f_oh)}")
        if f550d > 0:
            parts.append(f"550D:{_fmt(f550d)}")

        grade_line = " | ".join(parts)

        # Grade breakdown line only — no abbreviation
        # V9 was 13.75/2.5, +10% → 15.125/2.75
        if grade_line:
            ax_map.text(
                cx, cy, grade_line,
                fontsize=15.125, ha='center', va='center', fontweight='bold',
                color='#37474F',
                bbox=dict(facecolor='white', alpha=0.88, edgecolor='#90A4AE',
                          linewidth=0.7, pad=2.75, boxstyle='round,pad=0.3'),
                zorder=8,
            )
    ...
    for i, pname in enumerate(plant_order, 1):
        ...
        ax_map.text(disp_lon, disp_lat - 0.8, str(i),
                    fontsize=9, ha='center', va='top', fontweight='bold',
                    color='#1B2631',
                    bbox=dict(facecolor='white', alpha=0.85, edgecolor='#90A4AE',
                              linewidth=0.4, pad=1.5, boxstyle='round,pad=0.2'),
                    zorder=11)
    ...
    # ══════════════════════════════════════════════════════════════════
    # SIDEBAR — Color Key + Grade Breakdown only
    # ══════════════════════════════════════════════════════════════════
    y = 0.70
    ...
    for color, label in [(COLOR_550, "Fe 550"),
        ...
        ax_info.text(0.18, y, label, fontsize=13, color='#37474F',
                     transform=ax_info.transAxes, va='top')
        y -= 0.035

    ax_info.text(0.08, y, "Darker green = higher FY sales",
                 fontsize=11, color='#2E7D32', fontstyle='italic',
                 transform=ax_info.transAxes, va='top')
    ...
    plt.savefig(OUTPUT_FILE, dpi=300, bbox_inches='tight', facecolor='white',
                pad_inches=0.3)
    plt.close()
    print(f"V9 map saved: {OUTPUT_FILE}")
