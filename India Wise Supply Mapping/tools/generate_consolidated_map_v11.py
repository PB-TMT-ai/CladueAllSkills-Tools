"""
generate_consolidated_map_v11.py
V11 consolidated map — larger plant icons, adjusted plant positions.
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "consolidated_supply_map_v11.png")
...
def generate():
    ...
    # ── Compute plant-level grade totals (from BF / JSON) ──────────
    plant_grades = {}
    for pn, pd in plant_data.items():
        if pn not in PLANT_LOCATIONS:
            continue
        p550 = sum(pd.get("fe_550", {}).values())
        p_oh = sum(pd.get("one_helix", {}).values())
        p550d = sum(pd.get("fe_550d", {}).values())
        plant_grades[pn] = (p550, p_oh, p550d)

    # ── Factory icons + plant-level grade labels ─────────────────────
    factory_img = create_factory_icon(size=96)
    ...
    for i, pname in enumerate(plant_order, 1):
        ...
        # V11: zoom=1.0 (doubled from 0.5)
        imagebox = OffsetImage(factory_arr, zoom=1.0)
        ab = AnnotationBbox(imagebox, (disp_lon, disp_lat), frameon=False, zorder=10)
        ax_map.add_artist(ab)

        # Plant number label
        ax_map.text(disp_lon, disp_lat - 1.2, str(i),
        ...
        # Plant-level grade label
        p550, p_oh, p550d = plant_grades.get(pname, (0, 0, 0))
        parts = []
        if p550 > 0:
            parts.append(f"550:{_fmt(p550)}")
        if p_oh > 0:
            parts.append(f"OH:{_fmt(p_oh)}")
        if p550d > 0:
            parts.append(f"550D:{_fmt(p550d)}")
        grade_line = " | ".join(parts)
        if grade_line:
            ax_map.text(
                disp_lon, disp_lat + 1.6, grade_line,
                fontsize=10, ha='center', va='bottom', fontweight='bold',
                color='#37474F',
                bbox=dict(facecolor='white', alpha=0.90, edgecolor='#90A4AE',
    ...
    plt.savefig(OUTPUT_FILE, dpi=300, bbox_inches='tight', facecolor='white',
                pad_inches=0.3)
    plt.close()
    print(f"V11 map saved: {OUTPUT_FILE}")
