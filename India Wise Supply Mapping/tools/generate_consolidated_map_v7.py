"""
generate_consolidated_map_v7.py
V7 consolidated India map — premium design.
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "consolidated_supply_map_v7.png")
...
def generate():
    ...
    # ══════════════════════════════════════════════════════════════════
    # GRADE LABELS — clean stacked design, each grade on own line
    # ══════════════════════════════════════════════════════════════════
    GRADE_STYLE = {
        '550':  {'color': C550,  'prefix': '550'},
        'OH':   {'color': C_OH,  'prefix': 'OH'},
        '550D': {'color': C550D, 'prefix': '550D'},
    }
    LINE_H = 0.75       # vertical spacing between lines in map coords
    FONT_GRADE = 9       # font size for each grade line
    FONT_GRADE_VAL = 9   # font size for value
    ...
    for state in state_totals:
        ...
        n = len(lines)
        # Background card — draw a FancyBboxPatch
        card_w = 3.8   # degrees (approx width)
        card_h = n * LINE_H + 0.35
        card_x = cx - card_w / 2
        card_y = cy - card_h / 2
    ...
    plt.close()
    print(f"V7 map saved: {OUTPUT_FILE}")
