"""
generate_consolidated_map_v5.py
V5 consolidated map — based on corrected version + state-level grade labels.
- Choropleth: states colored by total supply volume (green gradient)
- Factory icons at plant locations with numbered labels
- State grade labels on map: each state shows 550 / OH / 550D volumes
...
OUTPUT_FILE = os.path.join(OUTPUT_DIR, "consolidated_supply_map_v5.png")
...
# ── State label positions (lon, lat) — tuned to avoid overlaps ────────
STATE_LABEL_POS = {
    ...
    "HARYANA": (74.0, 30.2),
    "PUNJAB": (73.0, 31.8),
    "UTTARAKHAND": (80.2, 31.0),
    "JAMMU & KASHMIR": (75.3, 34.3),
    "HIMACHAL PRADESH": (77.6, 32.8),
    "ODISHA": (84.0, 20.5),
    "MADHYA PRADESH": (78.6, 23.5),
    "MAHARASHTRA": (75.7, 19.5),
    "JHARKHAND": (85.3, 23.6),
    "WEST BENGAL": (88.2, 22.3),
    "BIHAR": (85.9, 25.6),
    "GUJARAT": (70.8, 22.3),
    "ANDHRA PRADESH": (79.7, 15.2),
    "TELANGANA": (79.0, 18.0),
    "ASSAM": (93.0, 26.1),
...
def generate():
    ...
    fig = plt.figure(figsize=(34, 24), facecolor='white')
    ...
    # ── State grade labels ───────────────────────────────────────────
    # Each state shows: "STATE_ABBREV" header + one line per grade that has volume
    # Format:  UP
    #          550: 72.1K | OH: 45.7K | 550D: 77.3K
    for state, total in state_totals.items():
        ...
        # State abbreviation (bold, larger)
        ax_map.text(
            cx, cy + 0.45, abbrev,
            fontsize=7.5, ha='center', va='center', fontweight='bold',
            color='#1B2631',
            bbox=dict(facecolor='white', alpha=0.88, edgecolor='#90A4AE',
                      linewidth=0.5, pad=1.5, boxstyle='round,pad=0.3'),
            zorder=8,
        )
        # Grade breakdown line (smaller, below abbreviation)
        if grade_line:
            ax_map.text(
                cx, cy - 0.45, grade_line,
    ...
    plt.savefig(OUTPUT_FILE, dpi=300, bbox_inches='tight', facecolor='white',
                pad_inches=0.3)
    plt.close()
    print(f"V5 map saved: {OUTPUT_FILE}")
