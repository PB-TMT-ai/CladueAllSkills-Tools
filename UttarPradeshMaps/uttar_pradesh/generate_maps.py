"""
Generate two interactive maps of Uttar Pradesh.
  1. District Potential Map (choropleth + stock yard pins + stockist pins + district labels)
  2. Distributor Territory Map (color-coded territories + dealer count circles + district labels)

Data source: Book 67 (1).xlsx (Sheet1, filtered by STATE_NAME)
GeoJSON:    uttar_pradesh_districts.geojson (local, uses 'district' property directly)

Outputs: uttar_pradesh_district_potential_map.html, uttar_pradesh_distributor_territory_map.html
"""
...
EXCEL_PATH   = os.path.join(os.path.dirname(BASE_DIR), "Book 67 (1).xlsx")
GEOJSON_PATH = os.path.join(BASE_DIR, "uttar_pradesh_districts.geojson")
MAP1_PATH    = os.path.join(BASE_DIR, "uttar_pradesh_district_potential_map.html")
MAP2_PATH    = os.path.join(BASE_DIR, "uttar_pradesh_distributor_territory_map.html")
...
STATE_BOUNDS = [[23.72, 76.94], [30.56, 84.78]]
STATE_CENTER = [27.14, 80.86]
...
DISTRIBUTOR_SHORT = {
    ...
    "OPTRIX INFRA LLP 2":                      "Optrix Infra 2",
...
def load_excel():
    ...
    # Hardcoded: Prayagraj is a stockyard location
