...
Generate two interactive maps of Uttar Pradesh:
  1. District Potential Map (green gradient choropleth + distributor markers)
  2. Distributor Territory Map (color-coded territories + labels)

Outputs: up_district_potential_map.html, up_distributor_territory_map.html
"""
...
MAP1_PATH = os.path.join(BASE_DIR, "up_district_potential_map.html")
MAP2_PATH = os.path.join(BASE_DIR, "up_distributor_territory_map.html")
...
def load_excel():
    ...
    district_potential = {}      # normalized_name → category
    district_distributors = {}   # normalized_name → [distributors]
    stockyard_districts = set()  # normalized names where stock yard = "Yes"
    district_dealers = {}        # normalized_name → dealer count (int)
    ...
    for row in rows:
        ...
        # First occurrence sets potential (all duplicates have same category)
        ...
        # Keep max dealer count if district appears multiple times
...
def create_territory_map(geojson_data, district_potential, district_distributors, centroids, district_dealers, areas):
    ...
    # District name labels + dealer counts (adaptive font size)
    label_group = folium.FeatureGroup(name="District Labels")
    for geo_name, centroid in centroids.items():
        excel_name = geojson_name_to_excel(geo_name)
        ...
        # Adaptive font sizing based on polygon area
        if area >= 0.8:
            name_size, dealer_size = 10, 9
        elif area >= 0.4:
            name_size, dealer_size = 9, 8
        else:
            name_size, dealer_size = 8, 7
...
def main():
    ...
    print("\n--- Generating Map 1: District Potential ---")
    ...
    print("\n--- Generating Map 2: Distributor Territories ---")
