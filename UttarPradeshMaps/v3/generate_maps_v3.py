"""
V3: Generate two interactive maps of Uttar Pradesh (tight-fit view).
  1. District Potential Map (choropleth + stock yard pins + stockist pins)
  2. Distributor Territory Map (color-coded territories + dealer count circles)

Changes from V2:
  - Stockist locations plotted on potential map (smaller green icons)
  - Prayagraj added as stockyard
  - Nikunj Udyog removed from legend (districts show as Unassigned)
  - Optrix Infra split into Optrix Infra 1 (West UP) and Optrix Infra 2 (East UP)

Outputs: up_district_potential_map_v3.html, up_distributor_territory_map_v3.html
"""
...
EXCEL_PATH = r"C:\Users\2750834\Downloads\Book 67 (6).xlsx"
GEOJSON_PATH = os.path.join(PARENT_DIR, "uttar_pradesh_districts.geojson")
MAP1_PATH = os.path.join(BASE_DIR, "up_district_potential_map_v3.html")
MAP2_PATH = os.path.join(BASE_DIR, "up_distributor_territory_map_v3.html")

# --- UP Bounding Box (with 0.15 deg padding) -------------------------------
...
# --- Stockist Locations (hardcoded) ----------------------------------------
# Districts where a stockist is ALREADY present (orange pins)
STOCKIST_EXISTING = {
    "Varanasi": "Already Appointed Stockist",
    "Prayagraj": "Already Appointed Stockist",
    "Ghaziabad": "2 Stockists",
    "Agra": "Dealer Stockist",
}

# Districts where a stockist is NEEDED (green pins)
STOCKIST_NEEDED = {
    "Ayodhya": "Distributor Stock",
    "Hardoi": "Dealer Stock",
    "Lakhimpur Kheri": "Dealer Stock",
    "Jaunpur": "Stockist",
    "Aligarh": "Dealer Stockist",
    "Moradabad": "Stockist",
}

# --- Optrix Infra Split (V3) -----------------------------------------------
OPTRIX_EAST_UP = {
    "Varanasi", "Prayagraj", "Jaunpur", "Mirzapur", "Pratapgarh",
    "Ghazipur", "Chandauli", "Bhadohi", "Sonbhadra", "Chitrakoot",
    "Kaushambi",
}
...
DISTRIBUTOR_COLORS = {
    ...
    "OPTRIX INFRA 1": "#377eb8",
    "OPTRIX INFRA 2": "#17becf",
...
DISTRIBUTOR_SHORT = {
    ...
    "OPTRIX INFRA 1": "Optrix Infra 1 (West)",
    "OPTRIX INFRA 2": "Optrix Infra 2 (East)",
...
def load_excel():
    ...
    # V3: Hardcode additional stockyard
    stockyard_districts.add("Prayagraj")

    # V3: Split "OPTRIX INFRA LLP" into OPTRIX INFRA 1 (West) / 2 (East)
    for norm, distribs in district_distributors.items():
        district_distributors[norm] = [
            ("OPTRIX INFRA 2" if norm in OPTRIX_EAST_UP else "OPTRIX INFRA 1")
            if d == "OPTRIX INFRA LLP"
            else d
            for d in distribs
        ]

    # Build reverse: distributor -> [districts]
...
def add_stockist_markers(m, centroids, stockyard_districts):
    """Add stockist location markers to a map, color-coded by status."""
    stockist_group = folium.FeatureGroup(name="Stockist Locations")

    # Combine both categories with a status tag
    all_stockists = [
        (district, label, "existing") for district, label in STOCKIST_EXISTING.items()
    ] + [
        (district, label, "needed") for district, label in STOCKIST_NEEDED.items()
    ]

    for district, stockist_type, status in all_stockists:
        if district not in centroids:
            continue
        centroid = centroids[district]

        # Larger offset for districts that also have a stockyard pin
        if district in stockyard_districts:
            offset_lat = centroid[0] - 0.10
            offset_lng = centroid[1] + 0.10
        else:
            offset_lat = centroid[0] - 0.04
            offset_lng = centroid[1] + 0.04
        ...
        folium.Marker(
            ...
            tooltip=f"Stockist ({status_label}): {district} ({stockist_type})",
            popup=f"<b>Stockist ({status_label})</b><br>{district}<br><i>{stockist_type}</i>",
        ).add_to(stockist_group)

    stockist_group.add_to(m)
...
def create_potential_map(geojson_data, district_potential, district_distributors, centroids, stockyard_districts):
    ...
    # Add stockist pins (V3)
    add_stockist_markers(m, centroids, stockyard_districts)
    ...
    <div style="position:fixed; bottom:30px; left:30px; z-index:1000;
        ...
        <div style="margin-bottom:4px;"><svg width="14" height="18" viewBox="0 0 14 18" style="vertical-align:middle;"><path d="M7 0C3.13 0 0 3.13 0 7c0 5.25 7 11 7 11s7-5.75 7-11c0-3.87-3.13-7-7-7z" fill="#e67e22" stroke="#333" stroke-width="0.5"/></svg>&nbsp; Stockist (Existing)</div>
        <div><svg width="14" height="18" viewBox="0 0 14 18" style="vertical-align:middle;"><path d="M7 0C3.13 0 0 3.13 0 7c0 5.25 7 11 7 11s7-5.75 7-11c0-3.87-3.13-7-7-7z" fill="#2e7d32" stroke="#333" stroke-width="0.5"/></svg>&nbsp; Stockist (Needed)</div>
    </div>
    """
...
def create_territory_map(geojson_data, district_potential, district_distributors, centroids, district_dealers, areas):
    ...
    # District name labels (adaptive font size)
    label_group = folium.FeatureGroup(name="District Labels")
    for geo_name, centroid in centroids.items():
        excel_name = geojson_name_to_excel(geo_name)
...
def main():
    ...
    print("\n--- Generating Map 1 (V3): District Potential ---")
    ...
    print("\n--- Generating Map 2 (V3): Distributor Territories ---")
